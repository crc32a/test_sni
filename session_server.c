...
int handle_client(sslcontainer_t *cnt) {
    ...
    BIO_printf(sni_b, "<body>\r\n");
    BIO_printf(sni_b, "    <html>\r\n");
    BIO_printf(sni_b, "        SNI = %s\r\n", sni);
    BIO_printf(sni_b, "        SERVER_IP = %s\r\n", cnt->server_ip);
    sess = SSL_get_session(cnt->ssl);
    if (char_to_hex(&hex, sess->session_id, sess->session_id_length)) {
        hex = "<ERROR>";
    }

    BIO_printf(sni_b, "        SESSION_ID = \"%s\" \n", hex);
    BIO_printf(sni_b, "        SESSiON_ID_LEN= %i\n", sess->session_id_length);
    free(hex);
    BIO_printf(sni_b, "    </html>\r\n");
    BIO_printf(sni_b, "</body>\r\n");
    BIO_flush(sni_b);
